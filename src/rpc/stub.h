...
namespace acc_engineer::rpc
{
    ...
    net::awaitable<void> datagram_stub<MethodChannel>::receiver_loop()
    {
        ...
        while (this->status_ == stub_status::running)
        {
            ...
            size_t size_read = co_await method_channel_.async_receive(segment_payload, net::use_awaitable);
    ...
    template<detail::async_datagram MethodChannel>
    template<detail::method_message Message>
    net::awaitable<detail::response_t<Message>> datagram_stub<MethodChannel>::async_call(const detail::request_t<Message> &request)
    {
    ...
    net::awaitable<void> stream_stub<MethodChannel>::sender_loop()
    {
        ...
        while (this->status_ == stub_status::running)
        {
            const std::string *buffers_to_send = co_await sender_channel_.async_receive(net::use_awaitable);
    ...
    template<detail::async_stream MethodChannel>
    template<detail::method_message Message>
    net::awaitable<detail::response_t<Message>> stream_stub<MethodChannel>::async_call(const detail::request_t<Message> &request)
    {
        return do_async_call<Message>(sender_channel_, request);
    }
}// namespace acc_engineer::rpc
