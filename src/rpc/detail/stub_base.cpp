...
namespace acc_engineer::rpc::detail
{
	...
	uint64_t stub_base::id() const { return stub_id_; }

	stub_status stub_base::status() const { return status_; }

	std::string stub_base::pack(uint64_t command_id, std::bitset<64> bit_flags,
	                            const Cookie& cookie,
	                            const std::string& message_payload) const
	{
		...
		payload_length = std::accumulate(
			payloads.begin(), payloads.end(), 0ULL,
			[](auto current, auto& buffer)
			{
	...
	std::tuple<uint64_t, std::bitset<64>, Cookie, std::string> stub_base::unpack(
		net::const_buffer payload) const
	{
		...
		buffer_copy(header_payload, payload,
		            sizeof(uint64_t) * header_payload.size());
		bit_flags = flags;
		payload += sizeof(uint64_t) * header_payload.size();

		if (!cookie.ParseFromArray(payload.data(),
		                           static_cast<int>(cookie_payload_size)))
		{
			throw sys::system_error(system_error::data_corrupted);
		}
	...
	uint64_t stub_base::generate_trace_id() { return trace_id_max_++; }

	net::awaitable<void> stub_base::dispatch(sender_channel_t& sender_channel,
	                                         net::const_buffer payload)
	{
		...
		if (bit_flags.test(flag_is_request))
		{
			co_spawn(
				...
				invoke_method(sender_channel, command_id, bit_flags, std::move(cookie),
		...
		else
		{
			...
			{
				spdlog::info(
					"{} dispatch_response message out of date, cmd_id: {} cookie: \"{}\"",
					this->id(), command_id, cookie.ShortDebugString());
				co_return;
			}

			co_await calling_[trace_id]->async_send({}, std::move(cookie),
			                                        std::move(message_payload),
			                                        net::use_awaitable);
		}
	}
	...
	net::awaitable<void>
	stub_base::invoke_method(sender_channel_t& sender_channel, uint64_t command_id,
	                         std::bitset<64> bit_flags, Cookie cookie,
	                         std::string message_payload)
	{
		spdlog::debug("{} invoke_method, cmd_id: {}, flags: {} cookie: \"{}\"", id(),
		              command_id, bit_flags.to_string(), cookie.ShortDebugString());

		std::string response_message_payload;
		try
		{
			response_message_payload = co_await std::invoke(
				method_group_, command_id, std::move(message_payload));
		}
		catch (const sys::system_error& e)
		{
			spdlog::error("{} invoke_method system error, code: {} what: \"{}\"", id(),
			              e.code().value(), e.what());
		...
		if (bit_flags.test(flag_no_reply)) { co_return; }
		...
		co_await sender_channel.async_send({}, std::move(payloads),
		                                   net::use_awaitable);
	}
}
