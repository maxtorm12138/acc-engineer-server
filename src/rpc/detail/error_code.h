...
public:
  const char* name() const noexcept final { return "rpc system error"; }
  ...
  default_error_condition(int code) const noexcept final {
    return {code, *this};
  }
};
...
template <>
struct boost::system::is_error_code_enum<
      acc_engineer::rpc::detail::system_error> : std::true_type {
};
