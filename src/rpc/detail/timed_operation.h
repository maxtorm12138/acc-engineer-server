...
template<typename T, typename Executor>
net::awaitable<std::optional<T>, Executor> awaitable_wrap(
    net::awaitable<T, Executor> a, std::enable_if_t<!std::is_constructible_v<T>, int> * = nullptr)
{
    co_return std::optional<T>(co_await std::move(a));
}

template<typename T>
T &awaitable_unwrap(std::conditional_t<true, T, void> &r, std::enable_if_t<std::is_constructible_v<T>, int> * = nullptr)
{
    return r;
}

template<typename T>
T &awaitable_unwrap(std::optional<std::conditional_t<true, T, void>> &r, std::enable_if_t<!std::is_constructible_v<T>, int> * = nullptr)
{
    return *r;
}
...
net::awaitable<void> timed_operation(Timer &timer, typename Timer::duration duration, net::awaitable<void, Executor> operation)
{
    ...
    auto parallel_group = net::experimental::make_parallel_group(
        co_spawn(ex, std::move(operation), net::deferred), co_spawn(ex, std::move(time_waiter), net::deferred));
...
net::awaitable<T> timed_operation(Timer &timer, typename Timer::duration duration, net::awaitable<T, Executor> operation)
{
    ...
    auto ex = co_await net::this_coro::executor;
    auto parallel_group = net::experimental::make_parallel_group(
        co_spawn(ex, detail::awaitable_wrap<T>(std::move(operation)), net::deferred), co_spawn(ex, std::move(time_waiter), net::deferred));
