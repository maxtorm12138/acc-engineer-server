...
net::awaitable<T, Executor> awaitable_wrap(net::awaitable<T, Executor> a,
                                           std::enable_if_t<
...
awaitable_wrap(net::awaitable<T, Executor> a,
               std::enable_if_t<!std::is_constructible_v<T>, int>* = nullptr) {
...
template <typename T>
T& awaitable_unwrap(
    std::optional<std::conditional_t<true, T, void>>& r,
    std::enable_if_t<!std::is_constructible_v<T>, int>* = nullptr) {
  return *r;
}

template <typename Executor, typename Timer>
net::awaitable<void> timed_operation(Timer& timer,
                                     typename Timer::duration duration,
                                     net::awaitable<void, Executor> operation) {
  ...
  auto [completion_order, ex0, ex1] = co_await parallel_group.async_wait(
      net::experimental::wait_for_one(), net::deferred);

  if (completion_order[0] == 0) {
    if (ex0 != nullptr) { std::rethrow_exception(ex0); }
...
net::awaitable<T> timed_operation(Timer& timer,
                                  typename Timer::duration duration,
                                  net::awaitable<T, Executor> operation) {
  auto time_waiter = [&timer, duration]() -> net::awaitable<void> {
    timer.expires_after(duration);
    co_await timer.async_wait(net::use_awaitable);
    co_return;
  ...
  auto ex = co_await net::this_coro::executor;
  auto parallel_group = net::experimental::make_parallel_group(
      co_spawn(ex, detail::awaitable_wrap<T>(std::move(operation)),
               net::deferred),
      co_spawn(ex, std::move(time_waiter), net::deferred));

  auto [completion_order, ex0, r0, ex1] = co_await parallel_group.async_wait(
      net::experimental::wait_for_one(), net::deferred);
