...
namespace acc_engineer::rpc
{
	...
	public:
		...
		method_group& implement(Implement&& implement)
		{
			...
			if (implements_.contains(command_id))
			{
				throw std::runtime_error(
					fmt::format("cmd_id {} already registered", command_id));
			}

			implements_.emplace(command_id,
			                    new detail::method<Message, Implement>(
				                    std::forward<Implement>(implement)));

			return *this;
		}

		net::awaitable<std::string> operator()(uint64_t command_id,
		...
		const
		{
			...
			{
				spdlog::error("method_group invoke error, no such implement cmd_id {}",
				              command_id);
				throw sys::system_error(detail::system_error::method_not_implement);
			}

			co_return co_await std::invoke(*implement->second,
			                               std::move(request_message_payload));
		}

	private:
		std::unordered_map<uint64_t, std::unique_ptr<detail::method_type_erasure>>
		implements_;
	};
}
