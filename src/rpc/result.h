...
template <detail::result_value T>
result<T>::result(const sys::error_code& error)
  : error_(error) {
}
...
template <detail::result_value T>
sys::error_code result<T>::error() const { return error_; }

template <detail::result_value T>
T& result<T>::value() {
  if (!value_.has_value()) { throw std::runtime_error("value empty"); }
  return *value_;
}

template <detail::result_value T>
const T& result<T>::value() const { return *value_; }
